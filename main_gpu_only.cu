...
int main() {
  ...
  int32_t i, j;
  int32_t sweep, ipass;
  StartEndPair *gpu0_pair = NULL, *gpu1_pair = NULL, *gpu2_pair = NULL, *gpu3_pair = NULL;
  int32_t color, loop_colored_sweeps;
  int32_t sweep_start, sweep_end, sweep_stride;
  FeedData *gpuData_0, *gpuData_1, *gpuData_2, *gpuData_3;
  gpuData_0 = (FeedData *)malloc(sizeof(FeedData));
  ...
  float *gpuDq_0, *gpuDq_1, *gpuDq_2, *gpuDq_3;

  int32_t *d_iam_0 = NULL, *d_jam_0 = NULL;
  double* d_res_0 = NULL;       //2d Array
  double* d_a_diag_lu_0 = NULL; //3dArray
  float* d_a_off_0 = NULL;      //3dArray
...
#pragma omp single nowait
    {
      gpuDq_0 = load_data("4-linear_system.dat.0", gpuData_0, unifiedMalloc);
    }
#pragma omp single nowait
    {
...
#pragma omp barrier
  ...
  load_data_on_gpu(gpuData_0, &d_iam_0, &d_jam_0, &d_res_0, &d_a_diag_lu_0, &d_a_off_0);
  ...
  printf("Solving Ax=b...\n");
  int32_t outer_sweeps = gpuData_0->outer_sweeps;
  ...
  for (i = 0; i < outer_sweeps; ++i)
    {
      ...
      loop_colored_sweeps = gpuData_0->max_colored_sweeps;

      sweep_start = 0;
      sweep_end = loop_colored_sweeps;
      sweep_stride = 1;

      if (gpuData_0->solve_backwards > 1 || gpuData_0->solve_backwards < -1) {
	sweep_start = loop_colored_sweeps - 1;
	sweep_end = -1;
	sweep_stride = -1;
      }

      if (gpuData_0->n_eqns <= 0) {
	sweep_start = 0;
	sweep_end = 1;
	sweep_stride = -1;
      }

      for (sweep = 0; sweep < gpuData_0->n_sweeps; ++sweep) { // sweeping
	for (color = sweep_start; color < sweep_end; color += sweep_stride) { // color_sweeps
	  for (ipass = 1; ipass <= 2; ++ipass) { // ipass
            ...
            #pragma omp parallel
	    {
              #pragma omp single nowait
	      {
		gpu0_pair = determineStartEnd(gpuData_0, color, ipass);
		dim3 numBlock_0((gpu0_pair->end - gpu0_pair->start + 1) / 4 + 1, 1, 1);
		// calling kernel for gpu computation
		solveOnGPU << <numBlock_0, blockSize, 0, stream0 >> > 
		  (gpuDq_0, d_res_0, d_iam_0, d_jam_0, d_a_off_0, d_a_diag_lu_0,
		   gpu0_pair->start, gpu0_pair->end, gpuData_0->solve_backwards, gpuData_0->nb);
	      }
              #pragma omp single nowait
	      {
                ...
                #pragma omp parallel
		{
                  #pragma omp single nowait
		  {
		    startDataTransfer(gpuDq_0, gpuData_0->dq_dim, gpuData_0->neqmax, 
				      &gpuData_0->sr[color]);
		  }
	...
	case 2:
	  {
	  buffer[0] = gpuData_0->sr[color].single_mat_senddata;
	  ...
	  sendproc[0] = gpuData_0->sr[color].sendproc;
	  ...
	  wtime2 = omp_get_wtime();
		{
                  #pragma omp single nowait
		  {
		    completeDataTransfer(gpuDq_0, gpuData_0->dq_dim, gpuData_0->neqmax, 
		                         &gpuData_0->sr[color], buffer, sendproc, 0);
		  }
  ...
  int32_t neq;
  rms1 = 0.0;
  for (i = 0; i < gpuData_0->neq0; i++) {
    for (j = 0; j < gpuData_0->dq_dim; j++) {
      rms1 = rms1 + pow(*(gpuDq_0 + i * gpuData_0->dq_dim + j), 2);
    }
  }
  ...
  neq = gpuData_0->neq0 + gpuData_1->neq0 + gpuData_2->neq0 + gpuData_3->neq0;
  rms_sum = sqrt(rms_sum / (double)neq / (double)gpuData_0->nb);

  if (isnan(rms_sum) || (rms_sum - gpuData_0->golden) >= tol) {
    printf("test failed: %.10E", rms_sum - gpuData_0->golden);
  }
  else {
    printf("test passed, result = %.10E\tgolden = %.10E\n", rms_sum, gpuData_0->golden);
  }
